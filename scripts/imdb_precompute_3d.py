...
def read_one_split(cfg, index_names, data_root_dir, output_dict, data_split = 'training', time_display_inter=100):
    ...
    for i, index_name in enumerate(index_names):
        ...
        min_z        = getattr(cfg.data, 'min_z', 0)
        max_trun     = getattr(cfg.data, 'max_trun', 1)
        min_pix_h     = getattr(cfg.data, 'min_pix_h', 0)
        if data_split == 'training':
...
def main(config:str="/home/zhangtianhan/codes/MonoDTR_plant/config/config.py"):
    cfg = cfg_from_file(config)
    torch.cuda.set_device(cfg.trainer.gpu)
    time_display_inter = 100 # define the inverval displaying time consumed in loop
    data_root_train_dir = cfg.path.train_path # the base directory of training dataset
    data_root_test_dir = cfg.path.test_path
    #now it is not useful
    # trian_label = os.path.join(data_root_train_dir,'Label')
    # test_label = os.path.join(data_root_test_dir,'Label')
    # list_label = os.listdir(trian_label)
    # N = len(list_label)
    ...
    train_names=os.listdir(os.path.join(data_root_train_dir,'RGB'))
    train_names=[os.path.splitext(file)[0] for file in train_names]
    val_names=os.listdir(os.path.join(data_root_test_dir,'RGB'))
    val_names=[os.path.splitext(file)[0] for file in val_names]
    read_one_split(cfg, train_names, data_root_train_dir, output_dict, 'training', time_display_inter)
    output_dict = {
                "calib": True,
                "image": False,
                "label": True,
    ...
    read_one_split(cfg, val_names, data_root_test_dir, output_dict, 'validation', time_display_inter)
