...
def compute_depth_for_split(cfg,
    ...
    depth_dir = os.path.join(save_dir, 'plant_depth')
    ...
    for i, index_name in tqdm(enumerate(index_names)):
        ...
        min_depth = 0
        max_depth = 8
        file_name = os.path.join(depth_dir, "%s.png"%index_name)
...
def main(config:str="/home/zhangtianhan/codes/MonoDTR_plant/config/config.py"):
   ...
   # N = len(list_calib)
    data_root_train_dir = cfg.path.train_path
    ...
    compute_depth_for_split(cfg, train_names, data_root_train_dir, output_dict, 'training', time_display_inter)
