...
class KittiMonoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split='training'):
        ...
        self.is_reproject = getattr(cfg.data, 'is_reproject', False) # if reproject 2d
    ...
    def _reproject(self, P2:np.ndarray, transformed_label:List[KittiObj]) -> Tuple[List[KittiObj], np.ndarray]:
        ...
        ###不清楚（y-0.5h）的意义；坐标到了底框上
        bbox3d_origin = torch.tensor([[obj.x, obj.y, obj.z, obj.w, obj.h, obj.l, obj.alpha] for obj in transformed_label], dtype=torch.float32)
        abs_corner, homo_corner, _ = self.projector(bbox3d_origin, bbox3d_origin.new(P2))
        for i, obj in enumerate(transformed_label):
            extended_center = np.array([obj.x, obj.y, obj.z, 1])[:, np.newaxis] #[4, 1]
            extended_bottom = np.array([obj.x, obj.y-0.5* obj.h, obj.z, 1])[:, np.newaxis] #[4, 1]
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            index_path=self.imdb[index].image2_path
            index_name,_= os.path.splitext(os.path.basename(index_path))
            if (ori_p2[0, 3] * transformed_P2[0, 3]) >= 0: # not mirrored or swaped, depth should base on pointclouds projecting through P2
                depth = cv2.imread(os.path.join(self.preprocessed_path, 'training', 'plant_depth', "%s.png"%index_name), -1)
