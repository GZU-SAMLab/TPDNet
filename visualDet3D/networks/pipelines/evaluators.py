...
def evaluate_kitti_obj(cfg:EasyDict, 
    ...
    result_texts = evaluate(
        label_path=os.path.join(cfg.path.test_path, 'Label'),
...
def test_one(cfg, index, dataset, model, test_func, backprojector:BackProjection, projector:BBox3dProjector, result_path):
    data = dataset[index]
    print(dataset.imdb[index].label2_path)
    file_name,_=os.path.splitext(os.path.basename(dataset.imdb[index].image2_path))
    ...
    if bbox.shape[1] > 4: # run 3D
        ...
        write_result_to_file(result_path, file_name, scores, bbox_2d, bbox_3d_state_3d, thetas, obj_names)
    else:
        ...
        write_result_to_file(result_path, file_name, scores, bbox_2d, obj_types=obj_names)
