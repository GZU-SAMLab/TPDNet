    ...
    def forward(self, bbox_3d, tensor_p2):
        """
            ...
            output:
                ...
                [N, ] thetas
        """
        #in plant datasets, we dont need to change bbox_3d's datas, only to calcualate corner points in image frame by P2
        """
        sage one :
        we dont use the data of angles(ry,alpha) to calcualate bbox_3d's information
        """
        ...
        # abs_corners=torch.zeros(bbox_3d.shape[0],8,3)
        # ### in plant dataset.codes:
        # h=bbox_3d[:,4]
        # l=bbox_3d[:,5]
        # w=bbox_3d[:,6]
        # x=bbox_3d[:,0]
        # y=bbox_3d[:,1]
        # z=bbox_3d[:,2]
        # bbox_x=[x-l/2,x+l/2,x+l/2,x-l/2,x-l/2,x+l/2,x+l/2,x-l/2]
        # bbox_y=[y-h/2,y-h/2,y-h/2,y-h/2,y+h/2,y+h/2,y+h/2,y+h/2]
        # bbox_z=[z+w/2,z+w/2,z-w/2,z-w/2,z+w/2,z+w/2,z-w/2,z-w/2]
        # abs_corners[:,:,0]=torch.stack([bbox_x[:, 0], bbox_x[:, 1], bbox_x[:, 1], bbox_x[:, 0], bbox_x[:, 0], bbox_x[:, 1], bbox_x[:, 1], bbox_x[:, 0]], dim=1)
        # abs_corners[:,:,1]=torch.stack([bbox_y[:, 0], bbox_y[:, 0], bbox_y[:, 0], bbox_y[:, 0], bbox_y[:, 4], bbox_y[:, 4], bbox_y[:, 4], bbox_y[:, 4]], dim=1)
        # abs_corners[:,:,2]=torch.stack([bbox_z[:, 2], bbox_z[:, 2], bbox_z[:, 3], bbox_z[:, 3], bbox_z[:, 2], bbox_z[:, 2], bbox_z[:, 3], bbox_z[:, 3]], dim=1)
        # camera_corners = torch.cat([abs_corners,
        #     abs_corners.new_ones([abs_corners.shape[0], self.corner_matrix.shape[0], 1])],
        #      dim=-1).unsqueeze(3)  # [N, 8, 4, 1]
        # camera_coord = torch.matmul(tensor_p2, camera_corners).squeeze(-1)  # [N, 8, 3]
        ...
        return abs_corners, homo_coord, None
