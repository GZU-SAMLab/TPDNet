...
class DepthAwareTransformer(nn.Module):
    ...
    def forward(self, depth_feat, context_feat, depth_pos=None):
        ...
        #integrated_feat=depth_feat+context_feat
...
class TransDecoderLayer(nn.Module):
    ...
    def forward(self, x, source):
        ...
        #Self-Attentiion for source (context_feat)
        ...
        result_source = source + self.drop_path(self.norm0(message))
        ...
        result_x = x + self.drop_path(self.norm1(message))

        #self-attetion for fusion
        fusion=result_x+result_source
        
        query,key,value = fusion,fusion,fusion

        query = self.q_proj2(query).view(bs, -1, self.nhead, self.dim)  # [N, L, (H, D)]
        key = self.k_proj2(key).view(bs, -1, self.nhead, self.dim)  # [N, L, (H, D)]
        value = self.v_proj2(value).view(bs, -1, self.nhead, self.dim)
        message = self.attention2(query, key, value)  # [N, L, (H, D)]
        message = self.merge2(message.view(bs, -1, self.nhead*self.dim)) 
        fusion= fusion+self.drop_path(self.norm2(message))

        fusion = fusion + self.drop_path(self.norm3(self.mlp(fusion)))
