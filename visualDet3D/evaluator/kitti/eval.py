...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = [
        'waxGourd', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'car',
        'tractor', 'trailer'
    ]
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
...
def calculate_iou_partly(gt_annos,
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            rots = np.zeros(np.concatenate([a["rotation_y"] for a in gt_annos_part], 0).shape)
            ...
            rots = np.zeros(np.concatenate([a["rotation_y"] for a in dt_annos_part], 0).shape)
